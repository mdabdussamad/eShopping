...
def search(request):
    ...
    if len(query)>78:
        ...
        productDescription =Product.objects.filter(description__icontains=query)
        productCategory =Product.objects.filter(category__icontains=query)       

        products=productTitle.union(productDescription, productCategory)
        # products = [item for item in Product.objects.all() if query in item.title or query in item.description or query in item.brand or query in item.category]
    if products.count()==0:
